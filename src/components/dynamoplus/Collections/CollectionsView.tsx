...
import { CollectionItemItem } from '../dynamoplus-dashboard/collection/CollectionItem/CollectionItem';
...
import { CheckOutlined, CloseOutlined, MenuOutlined, MinusCircleOutlined,PlusOutlined,ReloadOutlined,SearchOutlined,UnorderedListOutlined } from '@ant-design/icons';
      ...
      .then((res) => {
        if (isMounted.current) {
          ...
          setTableData({ data: res
            .splice(0,2)
            .map((c: Collection):CollectionTableRow=>{
            return {
              id_key: c.id_key || '',
              name: c.name,
      ...
      render: (text: string, record: { name: string; key: number }) => {
        return (
          <Space>
            ...
            <Link to={"/dynamoplus/collections/"+record.name}><Button type='ghost' icon={<SearchOutlined />}/></Link>
            </Tooltip>
  ...
  // };
    ...
    <S.Wrapper>
      ...
      <S.ButtonGroup>
        ...
        size='small'
        onClick={()=>{setIsCreateModalVisible(true)}}>{t('dynamoplus.collection.create')}</Button>
      <Button type='primary' icon={<ReloadOutlined />}
      size='small'>{t('dynamoplus.collection.refresh')}</Button>
      ...
      onClick={(e)=>{
          getCollections(3, tableData.data[tableData.data.length-1].name)
          .then((res)=>{
              const hasMore = res.length>2
              const newTableData = res
              .splice(0,2)
              .map((c: Collection):CollectionTableRow=>{
              return {
                id_key: c.id_key || '',
              ...
              setTableData({ data: [...tableData.data,...newTableData], 
                hasMore: hasMore, loading: false });
            
          });
          }}>{t('dynamoplus.collection.show_more')}</Button>
      </S.ButtonGroup>
