...
import { Collection as CollectionData, Index } from '@app/api/dynamoplus/mocks/system.api';
  ...
  const { t } = useTranslation();
    ...
    {indexes.map(i=>{
          ...
          <Descriptions title={i.name}  bordered layout='vertical' size='small' >
              <Descriptions.Item label={t('dynamoplus.index.index_configuration')} style={{backgroundColor: 'transparent'}}>
              ...
              <Descriptions.Item label={t('dynamoplus.index.conditions')} style={{backgroundColor: 'transparent'}}>
                {i.conditions.map(c=><S.Title>{c}</S.Title>)}
              </Descriptions.Item>
